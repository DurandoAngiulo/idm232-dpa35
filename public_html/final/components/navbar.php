<nav class="navbar navbar-expand-md bg-light shadow">
  <div class="container-fluid lightGrayBackground">
    <a class="navbar-brand fw-semibold fst-italic" href="#"><span class="red">Good</span> Eats</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
      <div class="navbar-nav container-fluid d-flex">
        <a class="nav-link" href="recipe.php">Meals</a>
        <a class="nav-link" href="#">Ingrediants</a>
        <a class="nav-link"href='#'>Occasions</a>
        <a class="nav-link me-auto" href='#'>News</a>
        <a class='nav-link' href="#"><img src="media/search.svg"></a>
        <a class="nav-link red" href='#'>Log In</a>
        <a class="nav-link" href='#'>Sign Up</a>
      </div>
    </div>
  </div>
</nav>